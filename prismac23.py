...
def collapse(g, bonuses=False, chaos=False, shapes=False, boom=False):
    ...
    # Decrement bomb fuses
    for r in range(ROWS):
        for c in range(COLS):
            if g[r][c] is not None and g[r][c].cell_type == CELL_BOMB:
                g[r][c].fuse = max(0, g[r][c].fuse - 1)
...
def collapse_shapes(g, bonuses=False, boom=False):
...
class Game:
    ...
    def build_title_widgets(self):
        ...
        specs = ((("ENDLESS", lambda: self.start_game(ENDLESS), GOLD),
                 ...
                 (("GRAPHICS", self.open_settings, (150, 160, 190)),
        ...
        sbox = self.settings_rect()
        self.setting_rows = []
        for i, (key, label, blurb) in enumerate(SETTING_DEFS):
            self.setting_rows.append(
                (key, pygame.Rect(sbox.x + 26, sbox.y + 76 + i * EXTRA_ROW,
                                  sbox.width - 52, EXTRA_ROW - 6), label, blurb))
        self.setting_buttons = [
            Button((sbox.x + 26, sbox.bottom - 62, sbox.width - 52, 44),
                   "BACK", self.close_settings),
        ]
    ...
    @staticmethod
    def settings_rect():
        return pygame.Rect(WIDTH // 2 - 230, 200, 460, 300)
    ...
    def draw_settings(self, screen):
        ...
        screen.blit(veil, (0, 0))
        box = self.settings_rect()
        screen.blit(translucent(box.size, PANEL_FILL, PANEL_EDGE, 16),
                    box.topleft)
        title = self.font_big.render("GRAPHICS", True, TEXT)
    ...
    def new_board(self):
        ...
        self.grid = self.apply_extras(new_grid(bonuses=self.timed,
    ...
    def build_widgets(self):
        ...
        half = (sw - 20) // 2
        self.menu_buttons = [
            Button((sx, box.y + 214, sw, 42), "RETURN TO TITLE",
                   self.return_to_title, accent=GOLD),
            Button((sx, box.y + 266, half, 42), "RESUME", self.close_menu),
            Button((sx + half + 20, box.y + 266, half, 42), "QUIT", self.quit,
                   accent=(232, 92, 92)),
        ]
        ...
        if self.on_title and self.settings_open:
            for key, rect, _, _ in self.setting_rows:
                if rect.collidepoint(pos):
                    ...
                    return
        elif self.on_title and self.extras_open:
    ...
    def on_motion(self, pos):
        if self.dragging is not None:
            ...
            return
        if self.on_title:
            active = (self.setting_buttons if self.settings_open
    ...
    def draw_motes(self, screen):
        ...
        for x, y, size, _, alpha in self.motes:
            pygame.draw.circle(layer, (150, 175, 255, int(alpha * 46)),
    ...
    def draw_scene(self, screen, background=True):
        ...
        if self.mode == SHAPES:
            # mask out the holes so the board reads as an irregular shape
            for r in range(ROWS):
                for c in range(COLS):
                    cell = self.grid[r][c]
                    if cell is not None and cell.cell_type == CELL_EMPTY:
                        screen.blit(
                            translucent((TILE, TILE), (0, 0, 0, 150), None, 0),
                            (BOARD_X + c * TILE, BOARD_Y + r * TILE))
