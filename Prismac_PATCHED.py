...
# see the lit circle.
# Increased difficulty: boom reduced from 1.45, others tightened slightly
MODIFIER_SCORE_FACTOR = {
    "mono": 1.00,
    "boom": 1.18,    # Reduced from 1.45 - explosives paid too much
    "lock": 0.20,    # Reduced from 0.22 - slightly harder
    "spotlight": 0.75,  # Reduced from 0.80 - slightly harder
    "chaos": 0.65,   # Reduced from 0.70 - slightly harder
}
...
# Slightly reduced to make gem goals harder
MODIFIER_GEM_FACTOR = {"chaos": 0.75, "spotlight": 0.80}
...
def campaign_level(number):
    ...
    if area == SPACE_AREA:
        ...
        # INCREASED DIFFICULTY: Space area is now significantly harder
        if kind == GOAL_GEMS:
            budget = max(10, 18 - step // 2)  # Reduced from 24 - step // 2
            per = (4.2 + step * 0.12) * modifier_factor(mods,
                                                        MODIFIER_GEM_FACTOR)
            target = int(round(budget * per / 2.0) * 2)
            return {"kind": kind, "target": target, "moves": budget,
                    "extras": mods,
                    "text": f"CLEAR {target} GEMS IN {budget} MOVES"}
        per_move = (220 + step * 16) * modifier_factor(mods,  # Increased from 170 + step * 12
                                                       MODIFIER_SCORE_FACTOR)
        if kind == GOAL_TIME:
            seconds = max(45, 60 - step)  # Minimum 45 seconds
        ...
        budget = max(8, 16 - step // 3)  # Reduced from 22 - step // 3
    ...
    # difference.
    # INCREASED DIFFICULTY: More points per move, fewer moves available
    per_move = 50 + area * 15 + step * 6  # Increased from 34 + area * 11 + step * 4

    # Check if this level has explosives - they need to be WAY harder
    has_explosives = "boom" in mods

    if kind == GOAL_GEMS:
        # Fewer moves available on all levels
        base_budget = 15 + area - step  # Reduced from 22 + area - step
        if has_explosives:
            base_budget = max(8, base_budget - 4)  # Even fewer moves for explosives
        budget = base_budget
    ...
    if kind == GOAL_TIME:
        ...
        target = int(round(seconds / 2.5 * per_move
                           * modifier_factor(mods, MODIFIER_SCORE_FACTOR)
                           / 50.0) * 50)
        # Explosives with timed goals need much higher score targets
        if has_explosives:
            target = int(target * 1.5)  # 50% higher score for explosives timed levels
        return {"kind": kind, "target": target, "seconds": float(seconds),
                "extras": mods,
                "text": f"SCORE {target:,} IN {seconds}S"}
    # Score goals with move limits - drastically reduced moves
    base_budget = 13 + area - step  # Reduced from 20 + area - step
    if has_explosives:
        base_budget = max(7, base_budget - 6)  # 7-13 moves for explosives (as requested)
    budget = base_budget
    target = int(round(budget * per_move
                       * modifier_factor(mods, MODIFIER_SCORE_FACTOR)
                       / 50.0) * 50)
    # Explosives levels need MUCH higher score targets
    if has_explosives:
        target = int(target * 1.8)  # 80% higher score requirement for explosives
    return {"kind": kind, "target": target, "moves": budget,
            "extras": mods,
            "text": f"SCORE {target:,} IN {budget} MOVES"}
