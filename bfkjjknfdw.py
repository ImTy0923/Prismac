...
class Game:
    ...
    def draw_settings(self, screen):
        ...
        screen.blit(title, (box.x + 28, box.y + 26))
        ...
        screen.blit(head, (box.x + 28, box.y + 190))
    ...
    def on_motion(self, pos):
        if self.dragging is not None:
            ...
            return
        if self.on_title:
            active = (self.extra_buttons if self.extras_open
    ...
    def draw_menu(self, screen):
        ...
        current = self.font_small.render("GRAPHICS", True, DIM)
        screen.blit(current, (box.x + 30, box.y + 194))
        for key, rect, label, blurb in self.setting_rows:
            on = self.settings.get(key, True)
            screen.blit(translucent(rect.size,
                                    GOLD + (60,) if on else (255, 255, 255, 18),
