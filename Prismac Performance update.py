...
class Puff:
    ...
    OPTIMIZATION: Pre-compute 12 rotation frames at startup instead of
    rotozoom every frame. Lookup is O(1) and massively faster.
    """

    __slots__ = ("image", "x", "y", "dx", "dy", "spin", "t", "life",
                 "scale0", "scale1", "delay", "tint", "_rotation_frames")

    # Class-level cache: key = id(image), value = list of 12 pre-rotated frames
    _rotation_cache = {}
    ROTATION_FRAMES = 12  # 30° per frame
    ...
    def __init__(self, image, x, y, angle=None, force=1.0):
        ...
        # Cache rotation frames for this image on first use
        img_id = id(image)
        if img_id not in Puff._rotation_cache:
            # Pre-compute 12 rotation frames (30° each)
            frames = []
            for i in range(self.ROTATION_FRAMES):
                angle_deg = (360.0 / self.ROTATION_FRAMES) * i
                rotated = pygame.transform.rotozoom(image, angle_deg, 1.0)
                frames.append(rotated)
            Puff._rotation_cache[img_id] = frames
        self._rotation_frames = Puff._rotation_cache[img_id]
    ...
    def draw(self, screen):
        ...
        # OPTIMIZATION: Look up pre-rotated frame instead of rotozoom
        rotation_index = int((self.spin * travel) / 30.0) % self.ROTATION_FRAMES
        rotated_base = self._rotation_frames[rotation_index]
        
        # Scale the pre-rotated frame
        if abs(scale - 1.0) > 0.01:
            new_size = int(rotated_base.get_width() * scale)
            image = pygame.transform.smoothscale(rotated_base, (new_size, new_size))
        else:
            image = rotated_base
        
        # Apply fade (alpha blend)
        if alpha < 255:
            image = image.copy()
            image.fill((255, 255, 255, int(alpha)), special_flags=pygame.BLEND_RGBA_MULT)
        
        # Apply tint if needed
        if self.tint < 255:
            if image is rotated_base:  # Only copy if we haven't already
                image = image.copy()
...
class ScorePop:
    ...
    def draw(self, screen, font):
        ...
        if self.is_rainbow:
            ...
            label.fill(color, special_flags=pygame.BLEND_RGBA_MULT)
...
class FlyingGem:
    ...
    OPTIMIZATION: Cache pre-rotated frames per unique image to avoid expensive
    rotozoom on every draw call during the level-end animation.
    """

    __slots__ = ("image", "x", "y", "dx", "dy", "spin", "delay", "t", "flat", "_rotation_frames")
    ...
    def __init__(self, image, x, y, delay, origin=None, force=1.0, flat=False):
        ...
        if img_id not in FlyingGem._rotation_cache:
            frames = []
            for i in range(self.ROTATION_FRAMES):
                angle_deg = (360.0 / self.ROTATION_FRAMES) * i
                rotated = pygame.transform.rotozoom(self.image, angle_deg, 1.0)
                frames.append(rotated)
            FlyingGem._rotation_cache[img_id] = frames
        self._rotation_frames = FlyingGem._rotation_cache[img_id]
    ...
    def draw_at(self, surface, place, scale):
        ...
        # OPTIMIZATION: Look up pre-rotated frame instead of rotozoom
        rotation_index = int((self.spin * p) / 30.0) % self.ROTATION_FRAMES
        rotated_base = self._rotation_frames[rotation_index]
        ...
        if fade < 255:
            image = image.copy()
            image.fill((255, 255, 255, fade), special_flags=pygame.BLEND_RGBA_MULT)
